...
class agentClient:
        ...
        def login(self) :
            ...
            # Send user credential authentication attempt to Master
                ...
                sioc.emit('usercredauth', [username, password], callback = self.loginresp)
            # Facial Recognition Login
                ...
                if os.path.exists("image.jpg"):
                    ...
                    sioc.emit('facerecauth', image, callback = self.loginresp)
                else :
                    print("No image found. Please place a selfie image labeled image.jpg in the directory")
                    time.sleep(1)
                    sioc.emit('reset', callback = self.login)
            elif option == "3":
                sioc.disconnect()
                sys.exit()
            else :
        ...
        def select_car(self, car_list) :
            ...
            try :
                ...
                if len(car_list[option]) > 0:
                    ...
                    time.sleep(2)
                    print("Car unlocked")
                    sioc.emit('carupdatestatus', [car['CarID'], 'Rented'], callback = self.location_update)
        ...
        def location_update(self) :
            ...
            while not self.exit_loop.is_set():
                ...
                sioc.emit('carupdatelocation', [self.car['CarID'], data])
            ...
            sioc.emit('carupdatestatus', [self.car['CarID'], 'Available'], callback = self.login)
